...
public class ExpressionNode {
	...
	public static ExpressionNode resolve(String expression) {
		...
		if (StrUtil.isBlank(expression)) {
			throw new IllegalValueExpressionException("Illegal value expression: \"%s\"; Pattern should not be blank", expression);
		}
		if (expression.indexOf(0) == '[' || expression.indexOf(0) == ']') {
			throw new IllegalValueExpressionException("Illegal value expression: \"%s\"; id should not be blank", expression);
		}
